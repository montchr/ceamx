...
(defun ceamx-ui-font-scaled-size (size)
  ...
  (alist-get size ceamx-ui-font-classic-scale-alist))

(defun ceamx-ui-font-scaled-size-float (size)
  "Return the float value for named font size SIZE."
  (* (ceamx-ui-font-scaled-size size) 0.01))

(defvar ceamx-ui-font-scaled-leading-divisor 20)
...
(package! fontaine
  (when (display-graphic-p)
    ...
    (setq fontaine-presets
          `((tiny :bold-weight medium
                  :default-height ,(pcase (system-name)
                                     (_ (ceamx-ui-font-scaled-size 'tiny))))

            (small :bold-weight bold
                   :default-height ,(pcase (system-name)
                                      (_ (ceamx-ui-font-scaled-size 'small)))
                   :default-weight ,(pcase (system-name)
                                      (_ 'regular)))

            (regular)

            (medium :default-height ,(pcase (system-name)
                                       ;; ("boschic" 124)
                                       (_ (ceamx-ui-font-scaled-size 'medium))))

            (large :default-height ,(pcase (system-name)
                                      (_ (ceamx-ui-font-scaled-size 'large))))

            (xlarge :default-height ,(pcase (system-name)
                                       (_ (ceamx-ui-font-scaled-size 'xlarge))))

            (huge :default-height ,(pcase (system-name)
                                     (_ (ceamx-ui-font-scaled-size 'huge))))

            (big-mclarge-huge :default-weight semilight
                              :default-height ,(pcase (system-name)
                                                 (_ (ceamx-ui-font-scaled-size 'big-mclarge-huge)))
